...
class Breadcrumb_Trail {
        ...
        if ( 0 < $item_count ) {
            ...
            foreach ( $this->items as $item ) {
                ...
                if ( is_singular() || is_page() ){
                    ...
                    global $post;
                    $medical_care_unit_link_item = get_permalink( $post->ID );

                }
                elseif ( is_archive() && ( !is_day() && !is_month() && !is_year() && !is_tag() ) ) {

                    $category = get_queried_object();
                    $medical_care_unit_link_item = get_category_link( $category->term_id );

                } elseif ( is_day() ) {

                    $medical_care_unit_link_item = get_day_link( get_the_time('Y'),get_the_time('m'),get_the_time('d') );

                } elseif ( is_month() ) {

                   $medical_care_unit_link_item = get_month_link( get_the_time('Y'),get_the_time('m') );

                } elseif ( is_year() ) {

                    $medical_care_unit_link_item = get_year_link( get_the_time('Y') );

                }elseif ( is_tag() ) {

                    $tag_id = get_queried_object()->term_id;
                    $medical_care_unit_link_item = get_tag_link( $tag_id );

                } elseif ( is_author() ) {

                    $medical_care_unit_link_item = get_author_posts_url( get_current_user_id() );

                }
                elseif ( is_search() ) {

                    $medical_care_unit_link_item = home_url();
                ...
                $item = ! empty( $matches )
                    ...
                    : sprintf( '<a href="'.esc_url( $medical_care_unit_link_item ).'" itemprop="item">%s</a>', $item );
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'medical-care-unit' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'medical-care-unit' ),
            'home'                => esc_html__( 'Home',                                  'medical-care-unit' ),
            'error_404'           => esc_html__( '404 Not Found',                         'medical-care-unit' ),
            'archives'            => esc_html__( 'Archives',                              'medical-care-unit' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'medical-care-unit' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'medical-care-unit' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'medical-care-unit' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'medical-care-unit' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'medical-care-unit' ),
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'medical-care-unit' ) ) );
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'medical-care-unit' ) ) );
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'medical-care-unit' ) ) );
    }
    ...
    protected function add_day_archive_items() {
        ...
        $medical_care_unit_year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'medical-care-unit' ) ) );
        $medical_care_unit_month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'medical-care-unit' ) ) );
        $medical_care_unit_day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'medical-care-unit' ) ) );

        // Add the year and month items.
        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y' ) ) ), $medical_care_unit_year );
        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) ), $medical_care_unit_month );

        // Add the day item.
        if ( is_paged() )
            $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y' ) ), get_the_time( 'm' ), get_the_time( 'd' ) ), $medical_care_unit_day );

        elseif ( true === $this->args['show_title'] )
            $this->items[] = $medical_care_unit_day;
    }
    ...
    protected function add_week_archive_items() {
        ...
        $medical_care_unit_year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'medical-care-unit' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'medical-care-unit' ) ) );
    ...
    protected function add_month_archive_items() {
        ...
        elseif ( true === $this->args['show_title'] )
            $this->items[] = $medical_care_unit_month;
    }
    ...
    protected function add_year_archive_items() {
        ...
        elseif ( true === $this->args['show_title'] )
            $this->items[] = $medical_care_unit_year;
    }
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'medical-care-unit' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'medical-care-unit' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'medical-care-unit' ) ) ) );
