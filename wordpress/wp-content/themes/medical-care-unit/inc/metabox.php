...
function medical_care_unit_category_add_form_fields_callback() {
    $medical_care_unit_image_id = null; ?>
...
function medical_care_unit_custom_create_term_callback($medical_care_unit_term_id) {
    // Check if 'category_custom_image_url' exists before using it
    if ( isset( $_REQUEST['category_custom_image_url'] ) ) {
        // Add term meta data
        add_term_meta(
            $medical_care_unit_term_id,
            'term_image',
...
function medical_care_unit_category_edit_form_fields_callback($ttObj, $taxonomy) {
    $medical_care_unit_term_id = $ttObj->term_id;
    $medical_care_unit_image = '';
    $medical_care_unit_image = get_term_meta( $medical_care_unit_term_id, 'term_image', true ); ?>
    <tr class="form-field term-image-wrap">
        <th scope="row"><label for="image"><?php esc_html_e('Image','medical-care-unit'); ?></label></th>
        <td>
        <?php if ( $medical_care_unit_image ): ?>
            <span id="category_custom_image">
               <img src="<?php echo $medical_care_unit_image; ?>" style="width: 100%"/>
            </span>
            <input type="hidden" id="category_custom_image_url" name="category_custom_image_url">
            <span>
...
function medical_care_unit_edit_term_callback($medical_care_unit_term_id) {
    // Check if 'category_custom_image_url' exists before using it
    if ( isset( $_POST['category_custom_image_url'] ) ) {
        $medical_care_unit_image = get_term_meta( $medical_care_unit_term_id, 'term_image' );
        if ( $medical_care_unit_image ) {
            update_term_meta( 
                $medical_care_unit_term_id, 
                'term_image', 
